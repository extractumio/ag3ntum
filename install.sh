...
generate_fernet_key() {
    ...
    # Fernet requires exactly 32 bytes of URL-safe base64-encoded key
...
# Fernet encryption key for encrypting user API keys in the database
# DO NOT change this after users have stored encrypted tokens
...
EOF
    ...
    # Add localhost to additional_allowed_hosts if using a different hostname
    if [[ "${SERVER_HOSTNAME}" != "localhost" ]]; then
        sed_inplace "s|additional_allowed_hosts: \[\]|additional_allowed_hosts: [\"localhost\", \"127.0.0.1\"]|" config/api.yaml
    fi
...
generate_external_mounts_yaml() {
    print_info "Generating external-mounts.yaml..."

    if [[ -f "config/external-mounts.yaml.example" ]]; then
        # Create a minimal version (no external mounts by default)
        cat > config/external-mounts.yaml << 'EOF'
_version: "0.2.0"

# =============================================================================
# External Mounts Configuration
# =============================================================================
# Configure external folder mounts for agent sessions.
# See external-mounts.yaml.example for full documentation.
#
# Mount Types:
...
per_user:
  ...
  rw: []
EOF
        print_success "external-mounts.yaml created (empty - configure as needed)"
    else
        print_warning "external-mounts.yaml.example not found, creating minimal config"
        cat > config/external-mounts.yaml << 'EOF'
_version: "0.2.0"
...
per_user:
  ...
  rw: []
EOF
        print_success "external-mounts.yaml created (minimal)"
    fi
}

generate_llm_api_proxy_yaml() {
    print_info "Generating llm-api-proxy.yaml..."

    if [[ -f "config/llm-api-proxy.yaml.example" ]]; then
        cp config/llm-api-proxy.yaml.example config/llm-api-proxy.yaml
        print_success "llm-api-proxy.yaml created (using defaults)"
    else
        print_warning "llm-api-proxy.yaml.example not found, creating minimal config"
        cat > config/llm-api-proxy.yaml << 'EOF'
_version: "0.2.0"
...
EOF
        print_success "llm-api-proxy.yaml created (minimal)"
    fi
}
...
generate_configuration() {
    ...
    [[ ! -f "config/external-mounts.yaml" ]] && missing_files+=("external-mounts.yaml")
    [[ ! -f "config/llm-api-proxy.yaml" ]] && missing_files+=("llm-api-proxy.yaml")
...
run_build() {
    ...
    if [[ -f "VERSION" ]]; then
        ...
        print_success "Version file created (.ag3ntum-version)"
    fi
}
