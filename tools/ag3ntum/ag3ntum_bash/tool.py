...
async def _bash_impl(
    ...
    *,
    ...
    """Core bash implementation - testable without MCP tool wrapper."""
    ...
    preview_lines: int = min(
        args.get("preview_lines", default_preview_lines),
        max_preview_lines
    )
    ...
    security_result: SecurityCheckResult = security_filter.check_command(command)
    ...
    try:
        ...
        if sandbox_executor is not None and sandbox_executor.config.enabled:
            ...
            try:
                ...
                logger.info(
                    f"Ag3ntumBash: SANDBOX ENABLED - wrapping in timeout({timeout_seconds}s, "
                    f"kill-after={kill_after_seconds}s) + bwrap: {command[:50]}..."
                )
            ...
            # timeout --kill-after=KILL sends SIGTERM first, then SIGKILL after grace period
            exec_command = f"timeout --kill-after={kill_after_seconds} {timeout_seconds} bash -c {shlex.quote(command)}"
            use_shell = True
        ...
        asyncio_timeout = timeout_seconds + kill_after_seconds + 30
        ...
        if (sandbox_executor is not None
                and sandbox_executor.linux_uid is not None
                and sandbox_executor.linux_gid is not None):
            logger.info(
                f"Ag3ntumBash: Bwrap will drop privileges to UID={sandbox_executor.linux_uid}, "
                f"GID={sandbox_executor.linux_gid}"
            )
        ...
        try:
            ...
            return _error_response(
                ...
                f"(forcibly killed after {kill_after_seconds}s grace period)"
            )
        ...
        if exit_code == 124:
            logger.warning(
                f"Ag3ntumBash: Command terminated by timeout after {timeout_seconds}s"
            )
            return _error_response(
                f"Command timed out after {timeout_seconds} seconds (SIGTERM sent)"
            )
        elif exit_code == 137:
            logger.warning(
                f"Ag3ntumBash: Command force-killed after {timeout_seconds}+{kill_after_seconds}s"
            )
            return _error_response(
                f"Command timed out and was force-killed after {timeout_seconds}s + "
                f"{kill_after_seconds}s grace period (SIGKILL sent)"
            )
...
""",
    ...
    async def bash(args: dict[str, Any]) -> dict[str, Any]:
        ...
        return await _bash_impl(
            ...
            timeout_seconds=bound_timeout,
            kill_after_seconds=bound_kill_after,
            default_preview_mode=bound_default_preview_mode,
            default_preview_lines=bound_default_preview_lines,
            max_preview_lines=bound_max_preview_lines,
            output_dir=bound_output_dir,
            sandbox_executor=bound_sandbox_executor,
            security_filter=bound_security_filter,
        )
