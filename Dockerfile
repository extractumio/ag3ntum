...
# Builds the production frontend bundle. The bundle is copied into the main
# image at /web_dist and served by the web container (src/web_frontend_server.py)
# in production mode. In dev mode, the Vite dev server is used instead.
FROM node:20-slim AS frontend-builder
WORKDIR /app
COPY src/web_terminal_client/package.json ./package.json
RUN npm install --no-fund --no-audit --no-package-lock --legacy-peer-deps
# Copy all frontend source (public/config.yaml generated by run.sh before docker build)
COPY src/web_terminal_client/ ./src/web_terminal_client/
WORKDIR /app/src/web_terminal_client
...
# Copy production frontend bundle from builder stage
COPY --from=frontend-builder /build/dist /web_dist
