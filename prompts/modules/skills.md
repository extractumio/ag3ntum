<!--
name: 'Module: Skills'
description: SDK-native skill discovery and usage for subagents
version: 1.0.0
variables:
  - SKILL_TOOL
  - TASK_TOOL
...
To see available skills, use the ${SKILL_TOOL} tool or list `.claude/skills/`.
...
- Example: To run deep research, use `${SKILL_TOOL}` tool with `skill: "deep-research"`, NOT `${TASK_TOOL}` tool with `subagent_type: "deep-research"`

## Slash Commands

Skills can be invoked using slash command syntax:
- `/<skill-name>` is shorthand to invoke a user-invocable skill
- When executed, the skill gets expanded to its full prompt and instructions

## Skill Best Practices

- Always read the skill's `SKILL.md` documentation before using it
- Use full paths when executing scripts: `python3 .claude/skills/<skill_name>/script.py`
- All output files must go to `./output/`
- The `.claude/skills/` directory is READ-ONLY - do not modify skill files
